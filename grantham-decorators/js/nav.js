(() => {
    ...
    const scrollManager = {
        ...
        handleScrollEffects() {
            ...
            if (elements.navigation && scrollManager.quoteBar) {
                if (scrollPosition > 150) {
                    // Past hero — hide nav, show quote bar
                    elements.navigation.classList.add('cs-hidden');
                    scrollManager.quoteBar.classList.add('qb-visible');
                } else {
                    // At top — show nav, hide quote bar
                    elements.navigation.classList.remove('cs-hidden');
                    scrollManager.quoteBar.classList.remove('qb-visible');
                }
            }
            scrollManager.lastScrollY = scrollPosition;
        },
    };
    ...
    const init = {
        inertState() {
            ...
            if (elements.navigation) {
                elements.navigation.querySelectorAll(CONFIG.SELECTORS.dropdownMenu).forEach((dropdown) => {
                    dropdown.inert = true;
                });
            }
        },
